...
program cans
...
#if defined(_DISTRIBUTED_TDMA)
  use mod_common_cudecomp, only: ap_z_dtdma
#if defined(_DTDMA_FAST)
...
#endif
  real(rp), allocatable, dimension(:,:,:) :: ap_d,cp_d,bp_d,p2_d,aap,bbp,ccp
  ...
  real(rp), allocatable, dimension(:,:,:) :: au_d,cu_d,av_d,cv_d,aw_d,cw_d
  logical :: is_dtdma_update_u,is_dtdma_update_v,is_dtdma_update_w
...
#if defined(_DTDMA_FAST)
  associate(n => ap_y%shape(:))
  allocate(aap(n(1),n(2),n(3)), &
           bbp(n(1),n(2),n(3)), &
           ccp(n(1),n(2),n(3)), &
           bp_d(n_z_d(1),n_z_d(2),n_z_d(3)), &
           p2_d(n_z_d(1),n_z_d(2),n_z_d(3)))
  end associate
#endif
#endif
...
#if defined(_DISTRIBUTED_TDMA)
  allocate(au_d(n_z_d(1),n_z_d(2),n_z_d(3)), &
           cu_d(n_z_d(1),n_z_d(2),n_z_d(3)), &
           av_d(n_z_d(1),n_z_d(2),n_z_d(3)), &
           cv_d(n_z_d(1),n_z_d(2),n_z_d(3)), &
           aw_d(n_z_d(1),n_z_d(2),n_z_d(3)), &
           cw_d(n_z_d(1),n_z_d(2),n_z_d(3)))
#endif
...
#if defined(_DTDMA_FAST)
  !$acc enter data create(bp_d,p2_d,aap,bbp,ccp) async
#endif
#endif
  !$acc wait
#if defined(_IMPDIFF)
...
#if defined(_DISTRIBUTED_TDMA)
  !$acc enter data create(au_d,cu_d,av_d,cv_d,aw_d,cw_d) async
#endif
  !$acc wait
#endif
#if defined(_OPENACC)
  ...
  do while(.not.is_done)
    ...
    do irk=1,3
      ...
      !$acc end kernels
#if !defined(_IMPDIFF_1D)
      call solver(n,ng,arrplanu,normfftu,lambdaxy,aa,bb,cc,cbcvel(:,:,1),['f','c','c'],u,au_d,cu_d,is_dtdma_update_u)
#else
      call solver_gaussel_z(ng,n                 ,aa,bb,cc,cbcvel(:,3,1),['f','c','c'],u,au_d,cu_d,is_dtdma_update_u)
#endif
      is_dtdma_update_u = .true.
      !$OMP PARALLEL WORKSHARE
      ...
      !$acc end kernels
#if !defined(_IMPDIFF_1D)
      call solver(n,ng,arrplanv,normfftv,lambdaxy,aa,bb,cc,cbcvel(:,:,2),['c','f','c'],v,av_d,cv_d,is_dtdma_update_v)
#else
      call solver_gaussel_z(ng,n                 ,aa,bb,cc,cbcvel(:,3,2),['c','f','c'],v,av_d,cv_d,is_dtdma_update_v)
#endif
      is_dtdma_update_v = .true.
      !$OMP PARALLEL WORKSHARE
      ...
      !$acc end kernels
#if !defined(_IMPDIFF_1D)
      call solver(n,ng,arrplanw,normfftw,lambdaxy,aa,bb,cc,cbcvel(:,:,3),['c','c','f'],w,aw_d,cw_d,is_dtdma_update_w)
#else
      call solver_gaussel_z(ng,n                 ,aa,bb,cc,cbcvel(:,3,3),['c','c','f'],w,aw_d,cw_d,is_dtdma_update_w)
#endif
      is_dtdma_update_w = .true.
#endif
      dpdl(:) = dpdl(:) + f(:)
      call bounduvw(cbcvel,n,bcvel,nb,is_bound,.false.,dl,dzc,dzf,u,v,w)
      call fillps(n,dli,dzfi,dtrki,u,v,w,pp)
      call updt_rhs_b(['c','c','c'],cbcpre,n,is_bound,rhsbp%x,rhsbp%y,rhsbp%z,pp)
      call solver(n,ng,arrplanp,normfftp,lambdaxyp,ap,bp,cp,cbcpre,['c','c','c'],pp,is_dtdma_update_p, &
                  ap_d,cp_d,bp_d,p2_d,aap,bbp,ccp)
